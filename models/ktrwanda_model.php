...
class Ktrwanda_Model extends Router {
    ...
    function ProcessRequest($orignal_req, $req_params) {
        ...
        $operator = 'ktrwanda';
        $req_params['operator']='ktrwanda';

        $req_response = $this->ManageHttpRequestSession($req_params,$orignal_req);

        $this->log->ExeLog($req_params, 'Ktrwanda_Model::Handler ManageRequestSession Returning Status ' . var_export($req_response,true), 2);
