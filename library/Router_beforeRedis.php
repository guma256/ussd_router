  ...
  function ManageRequestSession($req_params,$request){
   ...
   if($check ==0){
    ...
    $flag = $shortode;
     ...
     $req_params['shortcode']=$sc_data['record_id'];
     $this->SaveNewSession($req_params);
   $send_request =$this->SendExternalRequest($req_params,$request,$sc_data['route_url']);

   }else{
   //print_r($check);die();
   $send_request =$this->SendExternalRequest($req_params,$request,$check[0]['url']);
     }
   ...
   if(empty($send_request)&&$req_params['requesttype']=='cleanup'){
     ...
     $resp =$this->format->ParseXMLRequest($send_request);
